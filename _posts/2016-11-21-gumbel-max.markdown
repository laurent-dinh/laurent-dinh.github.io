...
= \big\{(u_1, u_2, \dots, u_K), \forall k' \leq K, ~~ -\log\big(-\log(u_{k'})\big) + \log(\alpha_{k'}) \leq -\log\big(-\log(u_{k})\big) + \log(\alpha_{k})\big\},
$$
...
![2-D example]({{ site.baseurl }}/img/C1C2.png "2-D example of how C1 and C2 partition the space configurations."){:height="600px"}
