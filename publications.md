---
layout: page
title: Reports
permalink: /reports/
---


### Density estimation using Real NVP
*Laurent&nbsp;Dinh*, [*Jascha&nbsp;Sohl-Dickstein*](http://www.sohldickstein.com/), [*Samy&nbsp;Bengio*](http://bengio.abracadoudou.com/)  
[**Deep Learning Symposium (Neural Information Processing Systems) 2016**](https://sites.google.com/site/nips2016deeplearnings/) **(Oral)**

Using invertible neural network for deep generative model with latent space, tractable log-likelihood, inference and sampling on large-scale. ([arxiv](http://arxiv.org/abs/1605.08803)) ([visualizations]({{ site.baseurl }}{% post_url 2016-07-12-real-nvp-visualization %}))

### Deep independence network analysis of structural brain imaging: A simulation study
[*Eduardo Castro*](http://mialab.mrn.org/lab-members/eduardo-castro.html), [*Devon Hjelm*](http://mialab.mrn.org/lab-members/devon.html), [*Sergey Plis*](http://www.mrn.org/people/sergey-plis/principal-investigators), *Laurent Dinh*, [*Jessica Turner*](http://www.mrn.org/people/jessica-turner/principal-investigators), [*Vince Calhoun*](http://www.mrn.org/people/http-www.mrn.org-people-vince-calhoun-principal-investigators/principal-investigators)  
[**IEEE 25th International Workshop on Machine Learning for Signal Processing 2015 (MLSP)**](http://mlsp2015.conwiz.dk/home.htm)

Studying the use of Non-linear Independent Components Estimation on simulated sMRI data.

### A Recurrent Latent Variable Model for Sequential Data
[*Junyoung&nbsp;Chung*](https://sites.google.com/site/junyoungchung08/), [*Kyle&nbsp;Kastner*](https://kastnerkyle.github.io/), *Laurent&nbsp;Dinh*, [*Kratarth&nbsp;Goel*](https://www.linkedin.com/in/kratarth-goel-98b37741), [*Aaron&nbsp;Courville*](https://aaroncourville.wordpress.com/), [*Yoshua&nbsp;Bengio*](http://www.iro.umontreal.ca/~bengioy/)  
[**Neural Information Processing Systems 2015**](https://nips.cc/Conferences/2015)

Variational auto-encoder meets Deep-RNN. ([arxiv](http://arxiv.org/abs/1506.02216)) ([code](https://github.com/jych/nips2015_vrnn))

### NICE: Non-linear Independent Components Estimation
*Laurent&nbsp;Dinh*, [*David&nbsp;Krueger*](http://www-etud.iro.umontreal.ca/~kruegerd/), [*Yoshua&nbsp;Bengio*](http://www.iro.umontreal.ca/~bengioy/)  
[**International Conference in Learning Representations 2015**](http://www.iclr.cc/doku.php?id=iclr2015:main) **workshop track**

Tractable maximum likelihood with deep neural networks. ([arxiv](http://arxiv.org/abs/1410.8516)) ([code](https://github.com/laurent-dinh/nice))

### Techniques for Learning Binary Stochastic Feedforward Neural Networks
[*Tapani&nbsp;Raiko*](http://users.ics.aalto.fi/praiko/), [*Mathias&nbsp;Berglund*](https://people.aalto.fi/index.html#mathias_berglund), [*Guillaume&nbsp;Alain*](http://www.cs.ubc.ca/~gyomalin/), *Laurent&nbsp;Dinh*  
[**International Conference in Learning Representations 2015**](http://www.iclr.cc/doku.php?id=iclr2015:main) **conference track**

Exploring gradient estimators for binary stochastic neural network. ([arxiv](http://arxiv.org/abs/1406.2989))

### Predicting Parameters in Deep Learning
[*Misha&nbsp;Denil*](http://mdenil.com), *Babak&nbsp;Shakibi*, *Laurent&nbsp;Dinh*, [*Kratarth&nbsp;Goel*](https://www.linkedin.com/in/kratarth-goel-98b37741), [*Marc'Aurelio&nbsp;Ranzato*](http://www.cs.toronto.edu/~ranzato/), [*Nando&nbsp;De&nbsp;Freitas*](http://www.cs.ox.ac.uk/people/nando.defreitas/)  
[**Neural Information Processing Systems 2013**](https://nips.cc/Conferences/2013)

Prior knowledge on models allows us to learn on a significantly smaller subset of parameters and predicting the remaining ones via kernel ridge regression. ([arxiv](http://arxiv.org/abs/1306.0543)) ([code](https://github.com/mdenil/parameter_prediction))